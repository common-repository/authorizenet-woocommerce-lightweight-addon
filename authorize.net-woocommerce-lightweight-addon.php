...
function authorizenet_lightweight_init()
{
	...
	if(class_exists('WC_Payment_Gateway'))
	{
		...
		$this->icon             = plugins_url( 'images/authorizenet_lightweight.png', __FILE__ );
		...
		$this->authorizenet_lightweight_description     = $this->get_option( 'authorizenet_lightweight_description');
		...
		(
			...
			'authorizenet_lightweight_description' => array(
		...
		$icon = '';
		if(is_array($this->authorizenet_lightweight_cardtypes ))
		{
        foreach ( $this->authorizenet_lightweight_cardtypes  as $card_type ) {
		...
		{
			$icon .= '<img src="'.esc_url( plugins_url( 'images/authorizenet_lightweight.png' , __FILE__ ) ).'" alt="Authorize.Net Payment Gateway" />';	  
		}
		...
		public function process_payment($order_id)
		{
			...
			$cardtype = $this->get_card_type(sanitize_text_field(str_replace(' ','',$_POST['authorizenet_lightweight-card-number'])));
		...
		if ( count($response_array) > 1 )
		{
			...
			{
				$wc_order->add_order_note( __( 'Response: '.$response_array[3], 'woocommerce' ) );
				wc_add_notice($response_array[3], $notice_type = 'error' );
			}
		}
		else 
		{
...
function authorizenet_lightweight_settings_link( $links ) {
    $settings_link = '<a href="admin.php?page=wc-settings&tab=checkout&section=wc_authorizenet_lightweight_gateway">' . __( 'Settings' ) . '</a>';
    array_push( $links, $settings_link );
  	return $links;
}
$plugin = plugin_basename( __FILE__ );
add_filter( "plugin_action_links_$plugin", 'authorizenet_lightweight_settings_link' );
